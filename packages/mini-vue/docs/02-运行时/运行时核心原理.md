...
### patchChildren 算法
patchChildren 算法是渲染器的核心算法，它负责对比新旧 VNode 的子节点并更新真实 DOM：

#### 整理流程
```mermaid
flowchart TD
    A[patchChildren] --> B["读取 c1、prevShapeFlag、c2、shapeFlag"]
    B --> C{新节点 shapeFlag 含 TEXT_CHILDREN?}
    C -->|是| D{旧节点 prevShapeFlag 含 ARRAY_CHILDREN?}
    D -->|是| E["卸载旧子节点（TODO）"]
    D -->|否| F
    E --> F{c2 !== c1?}
    F -->|是| G[hostSetElementText 更新文本]
    F -->|否| H[结束]
    G --> H
    C -->|否| I{旧节点 prevShapeFlag 含 ARRAY_CHILDREN?}
    I -->|是| J{新节点 shapeFlag 含 ARRAY_CHILDREN?}
    J -->|是| K[patchKeyedChildren]
    J -->|否| L["卸载旧子节点（TODO）"]
    I -->|否| M{旧节点 prevShapeFlag 含 TEXT_CHILDREN?}
    M -->|是| N[hostSetElementText 清空]
    M -->|否| O
    N --> O{新节点 shapeFlag 含 ARRAY_CHILDREN?}
    O -->|是| P["挂载新子节点（TODO）"]
    O -->|否| Q[结束]
    K --> Q
    L --> Q
...
**举例**：列表项带 `key`，旧子节点顺序为 `A → B → C → D`，更新后新顺序为 `B → A → D → C`。若双端对比无法在头尾「吃掉」相同节点，中间整段会进入乱序分支。

1. 用 `key` 在新数组里查位置：例如 `B` 在新下标 `0`，`A` 在 `1`，`D` 在 `2`，`C` 在 `3`。
2. 按**旧数组顺序**遍历，把每个新位置填上「对应旧节点的下标 + 1」（`+1` 是为了把「无对应旧节点(新增节点)」保留给 `0`）。得到  
   `newIndexToOldIndexMap = [2, 1, 4, 3]`，含义是：新序列第 0 位是旧里下标 1 的节点（B），第 1 位是旧里下标 0 的节点（A），第 2 位是旧里下标 3 的节点（D），第 3 位是旧里下标 2 的节点（C）。
3. 在这个数组上做 LIS（只看非 `0` 元素）：`[2, 1, 4, 3]` 的最长递增子序列可以取值为 `1, 3`，对应**新数组中的下标** `[1, 3]`，即节点 **A** 与 **C**。说明在新旧序列里，**A 始终在 C 左侧** 这一对相对顺序没变，这两个 DOM 不必再挪。
4. 最后从后往前处理时：下标落在 LIS 里的节点跳过 `move`，其余（本例中是 **B** 和 **D**）才插入到正确锚点前。这样可以把移动次数压到远小于「每个节点都盲移一遍」。
